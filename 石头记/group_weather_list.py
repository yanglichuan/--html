...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .city-name { font-size: 14px; font-weight: 600; margin-bottom: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .city-temp { font-size: 28px; font-weight: 300; }
        .city-weather { font-size: 12px; opacity: 0.8; display: flex; align-items: center; gap: 5px; }
        .city-meta { font-size: 10px; opacity: 0.5; margin-top: auto; display: flex; justify-content: space-between; }
...
js_logic = """
        ...
        // 3. Concurrency & Fetch Logic (Reused & Optimized)
        ...
        async function startFetching() {
            const poolSize = 6; 
            const queue = [...ALL_CITIES];
            const workers = [];
            
            for (let i = 0; i < poolSize; i++) {
                workers.push(delay(i * 200).then(() => worker(queue)));
            }
        ...
        async function worker(queue) {
            while (queue.length > 0) {
                ...
                await fetchCityWeatherWithRetry(city.adcode);
                await delay(200); 
            }
        }
        
        async function fetchCityWeatherWithRetry(adcode, retry = 0) {
            try {
                const res = await fetch(`https://restapi.amap.com/v3/weather/weatherInfo?key=${API_KEY}&city=${adcode}&extensions=base&output=JSON`);
                if (!res.ok) throw new Error(`HTTP ${res.status}`);
                const data = await res.json();
                
                if (data.status === '1' && data.lives && data.lives.length > 0) {
                    const live = data.lives[0];
                    weatherCache.set(adcode, live);
                    updateCard(adcode, live);
                    successCount++;
                } else {
                    if (data.info && (data.info.includes('LIMIT') || data.info.includes('QPS'))) throw new Error('LIMIT_EXCEEDED');
                    markError(adcode, '无数据');
                    failCount++;
                }
            } catch (e) {
                if (retry < 3) {
                    const waitTime = 1000 * Math.pow(2, retry);
                    document.getElementById('retry-badge').style.display = 'inline-block';
                    await delay(waitTime);
                    await fetchCityWeatherWithRetry(adcode, retry + 1);
                    setTimeout(() => document.getElementById('retry-badge').style.display = 'none', 1000);
                } else {
                    markError(adcode, '失败');
                    failCount++;
                }
            } finally {
                if (retry === 0) updateProgress();
            }
        }
